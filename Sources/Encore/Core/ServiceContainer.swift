...
internal final class ServiceContainer: @unchecked Sendable {
    ...
    init(configuration: Configuration) {
        ...
        let olapHttpClient = HTTPClient(baseURL: env.analyticsBaseURL, headers: ["X-Analytics-Key": configuration.apiKey])
        var oltpHttpClient: HTTPClientProtocol = HTTPClient(baseURL: env.apiBaseURL, headers: ["X-API-Key": configuration.apiKey])
        #if DEBUG
