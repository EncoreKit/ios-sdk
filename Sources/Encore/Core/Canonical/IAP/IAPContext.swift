...
struct IAPContext: Sendable {
    ...
    /// Trial length normalized to days. Sent to the backend so trial-end
    /// reminders schedule against the StoreKit-authoritative value rather
    /// than the portal-typed `iap_products.trial_duration_days`.
    ...
    init(from productInfo: IAPProductInfo?) {
        ...
        self.freeTrialDays = productInfo?.freeTrialDays
    }
