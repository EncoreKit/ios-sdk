...
class IAPClient {
    ...
    ///
    /// Always fires `IAPPurchasePresentingEvent` before invoking the
    /// path. On the delegated path we resolve product info from
    /// StoreKit so the event carries the same productName/price/type
    /// the native path tracks — gives the backend visibility into
    /// what the publisher's handler was about to present, even
    /// though we can't see what their handler actually shows.
    static func delegatePurchase(productId: String, placementId: String?, promoOfferId: String? = nil) async -> Bool {
        ...
        //    We fire `IAPPurchasePresentingEvent` with resolved product
        //    info BEFORE invoking the handler so partial flows (handler
        //    crash, network error mid-purchase, user cancel) still leave
        //    a trace of intent. `IAPHandlerInvokedEvent` rides alongside
        //    to carry handlerKind for funnel split.
        if let handler = Encore.shared.purchaseRequestBoolHandler {
            ...
            do {
                ...
                if success {
                    ...
                    // Pair the "presented" event with a matching "failed"
                    // for the cancel/false branch so the funnel closes.
                    analyticsClient?.track(
                ...
                analyticsClient?.track(
                    IAPPurchaseFailedEvent(
                        ...
                        price: info.price,
                        type: info.type,
                        reason: "handler_threw: \(error.localizedDescription)"
                    )
                )
                return false
            }
        ...
        //    Same presented/invoked telemetry shape as the bool path so
        //    the funnel reads consistently across handler kinds.
        if let handler = Encore.shared.purchaseRequestHandler {
            ...
            do {
                ...
                // Pair the "presented" event with a matching "failed"
                // event when the handler errors. reason identifies it as
                // a handler throw distinct from StoreKit failures.
                analyticsClient?.track(
                    IAPPurchaseFailedEvent(
                        productId: productId,
                        productName: info.name,
        ...
        // Native fallback fires its own `IAPPurchasePresentingEvent`
        // (with trigger="native") + failure tracking inside
        // `IAPClient.purchase(productId:)`, so we don't double-emit
        // here.
        do {
    ...
    /// Resolve product info from StoreKit for telemetry. Best-effort:
    /// returns empty strings when the App Store fetch fails (network,
    /// product not in catalog, etc.) so we never block the purchase
    /// flow on telemetry. Mirrors what `IAPClient.purchase` reads
    /// from `Product.products(for:)`.
