...
struct SDUISurfaceModifier: ViewModifier {
    ...
    private var roundedShape: AnyShape {
        if let cornerRadii {
            return AnyShape(UnevenRoundedRectangle(cornerRadii: cornerRadii.rectCornerRadii, style: .continuous))
        }
        return AnyShape(RoundedRectangle(cornerRadius: cornerRadius ?? 0, style: .continuous))
    }
...
struct SDUIGradientBorderModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if let border = border, !border.colors.isEmpty {
            ...
            content.overlay(
                RoundedRectangle(cornerRadius: border.cornerRadius ?? fallbackCornerRadius ?? 0, style: .continuous)
                    .strokeBorder(
                        LinearGradient(
                            colors: colors,
