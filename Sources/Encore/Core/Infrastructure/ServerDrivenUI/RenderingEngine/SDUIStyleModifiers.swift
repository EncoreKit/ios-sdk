...
struct SDUIBackgroundModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content.background(color?.resolved(in: appearance) ?? Color.clear)
    }
}
        ...
        if let width = width, let color = color {
            content.overlay(
                RoundedRectangle(cornerRadius: cornerRadius ?? 0)
                    .strokeBorder(color.resolved(in: appearance), lineWidth: width)
            )
        } else {
            content
        }
...
struct SDUIShadowModifier: ViewModifier {
    ...
    @Environment(\.sduiAppearance) private var appearance
    func body(content: Content) -> some View {
        let resolvedColor = shadow?.color.resolved(in: appearance) ?? Color.clear
