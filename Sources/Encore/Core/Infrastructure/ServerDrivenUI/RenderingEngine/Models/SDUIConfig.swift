...
///   `UIApplication.shared.open`. This is the default when the field is absent.
/// - `inAppBrowser`: present the link inside a full-screen in-app browser
///   (`SFSafariViewController`).
///
/// Decodes from the strings `"external"` / `"inAppBrowser"`.
enum SDUIOfferLinkPresentation: String, Decodable {
    case external = "external"
    case inAppBrowser = "inAppBrowser"

    /// Product decision: default to opening the actual browser when unset.
    static var `default`: SDUIOfferLinkPresentation { .external }
}
...
struct SDUIConfig: Decodable {
    ...
    /// `SDUIOfferLinkPresentation.default` (`.external`).
