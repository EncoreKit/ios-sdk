...
enum SDUIColor {
    ...
    /// `resolved(in:)`. Returns `.clear` for `.appearance` because it has no
    /// `Appearance` in scope — migrate those call sites to `resolved(in:)`
    /// when brand-aware theming matters.
    var color: Color {
        switch self {
    ...
    /// Resolves the color against an `Appearance`. Call this from any render
    /// site that has access to the active appearance (via `@Environment` in
    /// SwiftUI views/modifiers, or via `SDUIContext` in imperative renderers).
...
extension SDUIColor: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let hexValue = try? container.decode(String.self, forKey: .hex) {
            ...
            self = .appearance(appearanceToken)
        } else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "SDUIColor must have 'hex', 'semantic', or 'appearance' key"
                )
            )
        }
    }
