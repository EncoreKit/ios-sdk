...
/// A native one-shot confetti burst. SDUI JSON can't express particle systems,
/// so this element is a thin data shell that the renderer maps to a native
/// `CAEmitterLayer`-backed view (see `ConfettiView`). Used by the post-claim
/// "claimed" screen to rain celebratory confetti from the top.
///
