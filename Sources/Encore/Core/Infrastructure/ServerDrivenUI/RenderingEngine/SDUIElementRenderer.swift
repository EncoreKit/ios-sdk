        ...
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: isOn ? "checkmark.square.fill" : "square")
                    .foregroundColor(isOn ? context.resolveAccentColor() : Color(UIColor.tertiaryLabel))
                    .font(.title3)
...
private struct SDUISlideButtonView: View {
    ...
    private var displayText: String {
        isDisabled ? (config.disabledText ?? config.text) : config.text
    }
        ...
        GeometryReader { geo in
            ...
            ZStack(alignment: .leading) {
                ...
                RoundedRectangle(cornerRadius: trackHeight / 2)
                    .fill(config.trackColor?.color ?? Color(UIColor.tertiarySystemFill))
...
/// Text field input with local @State for per-keystroke editing.
/// Syncs to SDUIContext.values on every keystroke to enable dependent elements (e.g., slideButton).
@available(iOS 17.0, *)
