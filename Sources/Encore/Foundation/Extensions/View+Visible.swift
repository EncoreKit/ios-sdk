...
struct OptionallyOnVisible: ViewModifier {
    let action: (() -> Void)?
    /// `action` may only be nil for a reason that is constant for the life of
    /// the view: this branch IS an identity change, and flipping it rebuilds
    /// everything beneath.
