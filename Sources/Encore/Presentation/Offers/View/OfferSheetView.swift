...
struct OfferSheetView: View {
    ...
    private func sduiContent(_ loadedConfig: SDUIConfig) -> some View {
        ...
        .preferredColorScheme(preferredColorScheme)
        .fullScreenCover(item: $viewModel.safariWrapper) { wrapper in
    ...
    private func safariSheet(for wrapper: SafariURLWrapper) -> some View {
        // Presented via `.fullScreenCover` so the in-app browser covers the
        // whole screen (no 0.95 detent / drag indicator). Dismiss still routes
        // through `handleSafariDismiss()` to grant + transition.
