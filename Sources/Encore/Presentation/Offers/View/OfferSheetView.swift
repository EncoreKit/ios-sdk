    ...
    private func handleSubmitLead(action: SDUIAction) {
        ...
        guard !EmailValidation.isPrivateRelay(email) else {
            ...
            analyticsClient?.track(LeadPrivateRelayDetectedEvent(
                ...
                variantId: sduiContext.variantId
            ))
            return
        }
        ...
        // Submit lead: merge email into user attributes (synchronous, local-first)
        let remindToCancel = sduiContext.values["remindToCancel"] == "true"
        userManager?.setAttributes(UserAttributes(email: email))

        analyticsClient?.track(LeadCapturedEvent(
            email: email,
            remindToCancel: remindToCancel,
            presentationId: viewModel.presentationId,
            variantId: sduiContext.variantId,
            campaignId: sduiContext.currentOffer?.id
        ))

        Logger.info("📧 [Lead] Captured email, remindToCancel=\(remindToCancel)")

        // Transition to success state and chain its onEnter action (triggerIAP)
        if let successState = action.onSuccessState {
            if let onEnterAction = sduiContext.setState(successState) {
                handleAction(onEnterAction, offer: sduiContext.currentOffer)
            }
        }
    }
