...
class OfferSheetViewModel: ObservableObject {
    ...
    func setVariantMetadata(variantId: String?, context: SDUIContext) {
        ...
        context.setVariantMetadata(variantId: variantId, presentationId: presentationId, placementId: placementId)
    }
        ...
        Task { [weak self] in
            ...
            do {
                ...
                if Encore.shared.configuration?.unlock == .strict {
                    strictUnlockReconciler?.notePending(transactionId: transactionId, placementId: placementId, useCase: offerContext.useCase)
                }
    ...
    private func finishClaimFlow(claim: ClaimedOffer?) {
        ...
        Task { [weak self] in
            guard let self else { return }
            let outcome = await IAPClient.delegatePurchase(productId: iapProductId, placementId: placementId, presentationId: presentationId, useCase: offerContext.useCase)
    ...
    private func handleTriggerIAP(action: SDUIAction) {
        ...
        Task { [weak self] in
            let outcome = await IAPClient.delegatePurchase(productId: iapProductId, placementId: placementId, presentationId: presentationId, useCase: useCase)
