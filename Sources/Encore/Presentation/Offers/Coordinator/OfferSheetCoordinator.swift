...
internal final class OfferSheetCoordinator {
    ...
    private init(placementId: String, useCase: UseCase = .reduceChurn, copyOverrides: [String: String] = [:], presentationId: String = UUID().uuidString) {
    ...
    ///
    /// - Parameters:
    ///   - placementId: Publisher-defined placement identifier.
    ...
    static func present(
        ...
        let (result, presentationId) = await presentCore(placementId: placementId, useCase: useCase, copyOverrides: copyOverrides)
    ...
    private static func presentCore(
        ...
        enqueueDiagnostic(.showEntered(placementId: placementId, distinctId: EventEnvelope.resolveDistinctId(), variantId: variantId, useCase: useCase, presentationId: presentationId))
        ...
        guard #available(iOS 17.0, *) else {
            ...
            enqueueDiagnostic(.showAborted(reason: .unsupportedOS, placementId: placementId, distinctId: EventEnvelope.resolveDistinctId(), variantId: variantId, useCase: useCase, presentationId: presentationId, iosVersion: version))
        ...
        guard current == nil else {
            ...
            enqueueDiagnostic(.showAborted(reason: .alreadyPresenting, placementId: placementId, distinctId: EventEnvelope.resolveDistinctId(), variantId: variantId, useCase: useCase, presentationId: presentationId))
        ...
        switch NCLGhostTrigger.evaluate(
            useCase: useCase,
            logExposure: { NCLGhostTrigger.defaultLogExposure($0, variantId: variantId, useCase: useCase, placementId: placementId) }
        ) {
        case .suppress:
            ...
            enqueueDiagnostic(.showAborted(reason: .experimentControl, placementId: placementId, distinctId: EventEnvelope.resolveDistinctId(), variantId: variantId, useCase: useCase, presentationId: presentationId))
            return (.notPresented(.experimentControl), presentationId)
        case .proceed, .skipped:
            break
        }

        let coordinator = OfferSheetCoordinator(placementId: placementId, useCase: useCase, copyOverrides: copyOverrides, presentationId: presentationId)
    ...
    private func start() {
        guard let analyticsClient = analyticsClient,
            ...
            Self.enqueueDiagnostic(.showAborted(reason: .notConfigured, placementId: placementId, distinctId: EventEnvelope.resolveDistinctId(), variantId: sduiConfigManager?.variantId(for: useCase), useCase: useCase, presentationId: presentationId))
        ...
        let task = Task { [weak self] in
            ...
            analyticsClient.track(OfferPresentationTriggeredEvent(presentationId: presentationId, variantId: variantId, useCase: useCase, placementId: self.placementId))
            ...
            if sduiConfigManager?.layout(for: useCase) == nil {
                ...
                analyticsClient.track(
                    OfferPresentationFailedEvent(reason: NotPresentedReason.useCaseUnavailable, presentationId: presentationId, variantId: variantId, useCase: useCase, placementId: self.placementId)
                )
            ...
            do {
                ...
                guard !response.offerList.isEmpty else {
                    analyticsClient.track(
                        OfferPresentationFailedEvent(reason: NotPresentedReason.noOffers, presentationId: presentationId, variantId: variantId, useCase: useCase, placementId: self.placementId)
                    )
                ...
                if useCase.allowsIAP, sduiConfigManager?.layout(for: useCase)?.triggerIAPFirst == true {
                    ...
                    let outcome = await IAPClient.delegatePurchase(productId: iapProductIdForPurchase, placementId: self.placementId, presentationId: self.presentationId, useCase: useCase)
                ...
                analyticsClient.track(
                    OfferPresentationFailedEvent(reason: .error(error), presentationId: presentationId, variantId: variantId, useCase: useCase, placementId: self.placementId)
                )
                ...
                analyticsClient.track(
                    OfferPresentationFailedEvent(reason: .error(wrapped), presentationId: presentationId, variantId: variantId, useCase: useCase, placementId: self.placementId)
                )
    ...
    /// Enqueue a diagnostic event on the reliable outbox. No-op (silently
    /// dropped) when the SDK is not yet configured — additive, never throws.
    private static func enqueueDiagnostic(_ job: OutboxJob) {
        Encore.shared.services?.outbox.enqueue(job)
    }
