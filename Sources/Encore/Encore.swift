...
public protocol EncoreProtocol: AnyObject {
    ...
    func configure(apiKey: String, options: Encore.Options)
    ...
    @discardableResult
    func onPurchaseRequest(_ handler: @escaping (PurchaseRequest) async throws -> Bool) -> Self
    @discardableResult
...
public final class Encore: EncoreProtocol {
    internal static let sdkVersion: String = "1.4.50"
    ...
    /// Bool-returning purchase handler — preferred over the legacy throws-based handler.
    /// Returns true if purchase succeeded, false if cancelled/failed.
    /// App-level infrastructure — NOT cleared on `reset()`.
    internal var purchaseRequestBoolHandler: ((PurchaseRequest) async throws -> Bool)?
    ...
    /// Clears and re-prefetches offers for the current user.
    ...
    public func configure(apiKey: String, options: Options = .init()) {
        ...
        let config = Configuration(apiKey: apiKey, logLevel: options.logLevel, unlock: options.unlock)
        ...
        let initialVariantId = container.remoteConfigManager.ui?.variantId
        container.offers.prefetch(userId: initialUserId, attributes: container.user.userAttributes, variantId: initialVariantId)
    ...
    /// Backwards-compatible overload — forwards to `configure(apiKey:options:)`.
    @available(*, deprecated, message: "Use configure(apiKey:options:) instead")
    public func configure(apiKey: String, logLevel: LogLevel, unlock: UnlockMode = .optimistic) {
        configure(apiKey: apiKey, options: Options(logLevel: logLevel, unlock: unlock))
    }
    ...
    ///
    /// Return `true` when the purchase succeeds so Encore auto-dismisses the offer sheet.
    /// Thrown errors are caught by the SDK and treated as `false`.
    ///
    /// **Simple (most apps):**
    /// ```swift
    ...
    public func onPurchaseRequest(_ handler: @escaping (PurchaseRequest) async throws -> Bool) -> Encore {
        self.purchaseRequestBoolHandler = handler
        return self
    }

    /// Legacy overload — purchase is assumed successful if the handler returns without throwing.
    /// Prefer the Bool-returning overload for reliable dismiss behavior.
    @available(*, deprecated, message: "Use onPurchaseRequest returning Bool for reliable auto-dismiss")
    @discardableResult
