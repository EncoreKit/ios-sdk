...
public protocol EncoreProtocol: AnyObject {
    ...
    var isClaimOfferDisabled: Bool { get set }
...
public final class Encore: EncoreProtocol {
    internal static let sdkVersion: String = "1.4.28"
    ...
    /// When `true`, the claim offer button is grayed out and unclickable.
    /// Set to `true` to temporarily disable claiming (e.g., during loading states).
    /// Defaults to `false`.
    public var isClaimOfferDisabled: Bool = false
    ...
    public func identify(userId: String, attributes: UserAttributes? = nil) {
        ...
        let userChanged = userManager.identify(userId: userId, attributes: attributes)
    ...
    public func setUserAttributes(_ attributes: UserAttributes) {
        ...
        let mergedAttributes = userManager.setAttributes(attributes)
